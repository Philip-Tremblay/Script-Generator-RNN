...
<span class="sd">    &quot;&quot;&quot;</span>
    ...
    <span class="c1">#Get a count and a list of unique words. Sorted in descending frequency</span>
    ...
    <span class="c1">#Did not like this as it itterated the list twice</span>
    <span class="c1">#vocab_to_int = {word: i for i, word in enumerate(vocab)}</span>
    <span class="c1">#int_to_vocab = {i: word for i, word in enumerate(vocab)}</span>
    
    <span class="n">vocab_to_int</span> <span class="o">=</span> <span class="p">{}</span>
    <span class="n">int_to_vocab</span> <span class="o">=</span> <span class="p">{}</span>
    <span class="k">for</span> <span class="n">i</span><span class="p">,</span> <span class="n">word</span> <span class="ow">in</span> <span class="nb">enumerate</span><span class="p">(</span><span class="n">vocab</span><span class="p">):</span>
        <span class="n">vocab_to_int</span><span class="p">[</span><span class="n">word</span><span class="p">]</span> <span class="o">=</span> <span class="n">i</span>
        <span class="n">int_to_vocab</span><span class="p">[</span><span class="n">i</span><span class="p">]</span><span class="o">=</span><span class="n">word</span>
...
<span class="sd">    &quot;&quot;&quot;</span>
    ...
    <span class="c1">## The sequences are to be a rolling list with the next item being the target</span>
    <span class="c1"># Total number of sequences</span>
    <span class="n">n_seq</span> <span class="o">=</span> <span class="nb">len</span><span class="p">(</span><span class="n">words</span><span class="p">)</span><span class="o">-</span><span class="n">sequence_length</span>
    
    <span class="c1"># Get empty array and vector as expected for the dataset</span>
    <span class="n">features</span> <span class="o">=</span> <span class="n">np</span><span class="o">.</span><span class="n">zeros</span><span class="p">((</span><span class="n">n_seq</span><span class="p">,</span> <span class="n">sequence_length</span><span class="p">),</span> <span class="n">dtype</span><span class="o">=</span><span class="nb">int</span><span class="p">)</span>
    <span class="n">targets</span>  <span class="o">=</span> <span class="n">np</span><span class="o">.</span><span class="n">zeros</span><span class="p">((</span><span class="n">n_seq</span><span class="p">),</span> <span class="n">dtype</span><span class="o">=</span><span class="nb">int</span><span class="p">)</span>
    
    <span class="c1"># Roll through all the words, saving the next word as the target</span>
    <span class="k">for</span> <span class="n">i</span> <span class="ow">in</span> <span class="nb">range</span> <span class="p">(</span><span class="n">n_seq</span><span class="p">):</span>
        <span class="n">features</span><span class="p">[</span><span class="n">i</span><span class="p">]</span> <span class="o">=</span> <span class="n">words</span><span class="p">[</span><span class="n">i</span><span class="p">:</span><span class="n">i</span><span class="o">+</span><span class="n">sequence_length</span><span class="p">]</span>
        <span class="n">targets</span> <span class="p">[</span><span class="n">i</span><span class="p">]</span> <span class="o">=</span> <span class="n">words</span><span class="p">[</span><span class="n">i</span><span class="o">+</span><span class="n">sequence_length</span><span class="p">]</span>

    <span class="c1"># Build the data set and loader. Shuffle is set true in the loader</span>
    <span class="n">dataset</span> <span class="o">=</span> <span class="n">TensorDataset</span><span class="p">(</span><span class="n">torch</span><span class="o">.</span><span class="n">from_numpy</span><span class="p">(</span><span class="n">features</span><span class="p">),</span> <span class="n">torch</span><span class="o">.</span><span class="n">from_numpy</span><span class="p">(</span><span class="n">targets</span><span class="p">))</span>
    <span class="n">dataloader</span> <span class="o">=</span> <span class="n">DataLoader</span><span class="p">(</span><span class="n">dataset</span><span class="p">,</span> <span class="n">shuffle</span><span class="o">=</span><span class="kc">True</span><span class="p">,</span> <span class="n">batch_size</span><span class="o">=</span><span class="n">batch_size</span><span class="p">)</span> 
    <span class="c1">#dataloader = DataLoader(dataset, batch_size=batch_size) # for testing</span>
...
torch.Size([10])
tensor([ 47,  17,  30,  48,   9,  32,   8,  16,  34,  40])
</pre>
</div>
</div>
...
<span class="sd">        &quot;&quot;&quot;</span>
        ...
        <span class="n">dropout_out</span> <span class="o">=</span> <span class="bp">self</span><span class="o">.</span><span class="n">dropout</span><span class="p">(</span><span class="n">lstm_out</span><span class="p">)</span>
        <span class="n">output</span> <span class="o">=</span> <span class="bp">self</span><span class="o">.</span><span class="n">fc</span><span class="p">(</span><span class="n">dropout_out</span><span class="p">)</span>
        
        <span class="c1">## reshape into (batch_size, seq_length, output_size)</span>
        ...
        <span class="c1"># return one batch of output word scores and the hidden state</span>
...
<span class="sd">        &#39;&#39;&#39;</span>
        ...
        <span class="c1"># initialize hidden state with zero weights, and move to GPU if available</span>
...
<span class="sd">    :param rnn: The PyTorch Module that holds the neural network</span>
<span class="sd">    :param optimizer: The PyTorch optimizer for the neural network</span>
...
<span class="sd">    &quot;&quot;&quot;</span>
    ...
    <span class="c1"># perform backpropagation and optimization</span>
    
    <span class="c1"># Creating new variables for the hidden state, otherwise would </span>
    <span class="c1"># backprop through the entire training history</span>
    ...
    <span class="n">target</span> <span class="o">=</span> <span class="n">target</span><span class="o">.</span><span class="n">float</span><span class="p">()</span><span class="o">.</span><span class="n">to</span><span class="p">(</span><span class="n">dtype</span><span class="o">=</span><span class="n">torch</span><span class="o">.</span><span class="n">long</span><span class="p">)</span>
    <span class="n">loss</span> <span class="o">=</span> <span class="n">criterion</span><span class="p">(</span><span class="n">output</span><span class="o">.</span><span class="n">squeeze</span><span class="p">(),</span> <span class="n">target</span><span class="p">)</span>
    
    <span class="n">loss</span><span class="o">.</span><span class="n">backward</span><span class="p">()</span>
    <span class="c1"># `clip_grad_norm` helps prevent the exploding gradient problem in RNNs / LSTMs.</span>
    <span class="n">nn</span><span class="o">.</span><span class="n">utils</span><span class="o">.</span><span class="n">clip_grad_norm_</span><span class="p">(</span><span class="n">rnn</span><span class="o">.</span><span class="n">parameters</span><span class="p">(),</span> <span class="n">clip</span><span class="p">)</span>
    <span class="n">optimizer</span><span class="o">.</span><span class="n">step</span><span class="p">()</span>
    
    <span class="c1"># return the loss over a batch and the hidden state produced by our model</span>
    <span class="k">return</span> <span class="n">loss</span><span class="o">.</span><span class="n">item</span><span class="p">(),</span> <span class="n">hidden</span>
...
<li>Set <code>vocab_size</code> to the number of unique tokens in our vocabulary.</li>
...
<span class="n">sequence_length</span> <span class="o">=</span> <span class="mi">10</span>  <span class="c1"># of words in a sequence</span>
...
Epoch:   39/40    Loss: 3.2856813545227053

Epoch:   40/40    Loss: 3.250826999826251

Epoch:   40/40    Loss: 3.254829053401947

Epoch:   40/40    Loss: 3.283484868526459
...
<pre>/opt/conda/lib/python3.6/site-packages/torch/serialization.py:193: UserWarning: Couldn&#39;t retrieve source code for container of type RNN. It won&#39;t be checked for correctness upon loading.
...
Began with the suggested values from the section on hyperparameters. Learning rate at 0.01 and the settings for a question answering network. Two layers, 500 as the embeded dimension, 300 as the hidden dimension.</p>
<p>I almost immediatly reduced the learning rate to 0.001 after seeing that the loss was climbing.</p>
<p>At 0.001, the loss decreased to about 4.4 after 2 epochs but bounced around here quite a bit. I decided to see what 0.0001 would do.</p>
<p>Progress was slow but steady, breaking below 4.25 in the third epoch and hitting 4.21 in the fourth epoch.</p>
<p>Leaving the learning rate at 0.0001, I increased the number of LSTM layers from 2 to 3. Again, the first result was 6.6 after 500 batches. Then progressed at a very very slow rate. I opted to stope early and bump the learning rate back up to 0.001.</p>
<p>Showing a steady deacrease in loss as with 2 layers and a learning rate of 0.0001. However, after a couple epochs once again seems to stall at 4.4.</p>
<p>Decrease the number of layers to 2, kept the learning rate at 0.001. This time increasing the hidden dimension and embedding dimension to be equal at 600. Hmm, seems a little worst. Let's try setting both at 300? Still stalling at around 4.4. Returning hidden dimension and embedding dimension to 600, 300 respectively.</p>
<p>Better, I was just reading about the effects of increasing the sequen length. Interesting. Icreasing from 20 to 32. Not a whole lot of difference. Increasing the batch size from 32 to 64. Looks like this is making a difference, loss is now below 4.2. Increasing to 128 has made even more improvement.</p>
<p>Had to stop testing do to sleep. :) Decreased the sequence length to 10. This makes more sense then looking at 20 words to create the next word. Could be reduced more? I think the next thing to try is doubling the batch size to 256 though this may chanllange the amount of RAM available. Next, with this larger batch size, I'd try increasing the size of the embedded and hidden layers.</p>
<p>After 5 epochs the loss made it below 4.0 and may even make the goal of 3.5 if I left it training long enough, though doubtful as the rate of decrease was very much flattening. Increased the batch size to 256 and set the maximum number of epochs to 20 from 10 to adjust from the reduced number of iterations.</p>
<p>Of all the variables, increasing the batch size has had the most dramatic effect on the training. After 15 epochs, the loss seems to have flattened out at 3.65. Getting close! Next increasing the batch size to 512 and the number of epochs to 40.</p>
<p>It appears that 40 was close to the number of epochs needed, though it seems even more would yield even more improvement. Is there a utility to dynamically calculate epochs or I could code one up?. I suspect reducing the sequence size to 8 from 10 would yield a lower loss and better performance. Though, the current settings satisfied the goal of a loss less than 3.5 with a final loss of 3.28. However, I would like to test for over fitting even with the dropout at 0.5.</p>
...
<div class="output_subarea output_stream output_stderr output_text">
<pre>/opt/conda/lib/python3.6/site-packages/ipykernel_launcher.py:49: UserWarning: RNN module weights are not part of single contiguous chunk of memory. This means they need to be compacted at every call, possibly greatly increasing memory usage. To compact weights again call flatten_parameters().
</pre>
</div>
</div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>jerry: robin&#39;s one who got the big salad.
